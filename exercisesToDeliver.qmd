...
# Tasa de coalescencia para cada T_i
p3 <- 6/(4*N)
p2 <- 2/(4*N)
lambda3 <- choose(3, 2) / (2 * N)  # para T3
lambda2 <- choose(2, 2) / (2 * N)  # para T2
...
cat("Esperanza teórica de T_MRCA =", E_TMRCA, "\n")

# i. Simulaciones de T_MRCA
set.seed(123)  # reproducibilidad
T3 <- rgeom(50, prob = lambda3)
T2 <- rgeom(50, prob = lambda2)
T_MRCA <- T3 + T2

# Comparar con la esperanza teórica
mean(T_MRCA)
hist(T_MRCA,
     main = "Distribución simulada de T_MRCA",
     xlab = "Tiempo (en generaciones)",
     col = "lightblue", border = "white")
abline(v = E_TMRCA, col = "red", lwd = 2, lty = 2)
legend("topright", legend = c("Esperanza teórica"), col = "red", lty = 2)
