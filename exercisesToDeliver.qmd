...
El Site Frequency Spectrum (SFS) es una distribución de frecuencias que resume la variabilidad genética observada en una población o muestra de secuencias de ADN. En particular, SFS muestra cuantos sitios segregantes presentan el alelo derivado en $1, 2, 3...$ hasta $n-1$  copias dentro de una muestra de tamaño $n$. De esta manera, el SFS describe la frecuencia de las mutaciones según cuantas copias derivadas tienen, lo que permite estudiar procesos evolutivos como es la deriva y la selección.

La esperanza bajo el modelo neutral estándar se define como $\mathbb{E}[S_i] = 4N \mu \cdot \frac{1}{i}$

donde:

$S_i$ el numero de sitios esperados con el alelo derivado,
$4N\mu$ diversidad genética, combina tamaño poblacional efectivo $(N)$ y la tasa de mutación por sitios $(\mu)$
$\frac{1}{i}$ factor del tiempo esperado en un momento que hay $i$ linajes en la genealogía.

## b. Mostrar como se obtienen las esperanzas para muestras de alelos de tamano n=2 y n=3.

Para $n=2$ solo hay una rama interna que puede diferenciarse.
Toda mutación que se encuentra en rama externa es un singleton, solo una copia tendrá la mutación.

$\mathbb{E}[S_2​]= 4N \mu = θ$
...
legend("topright", legend = paste("E[S] teórico =", round(nS,2)), col = "red", lty = 2)
...
# hist(T2,
#      main = "Simulación de T2 (n=2, 50 replicaciones)",
#      xlab = "Tiempo hasta coalescencia (T2)",
#      col = "lightblue", border = "white")
# abline(v = mean(T2), col = "blue", lwd = 2)
# abline(v = 2*N, col = "red", lwd = 2, lty = 2)
# legend("topright", legend = c("Media simulada", "Esperanza teórica"),
