...
Siendo $\frac{1}{i}$ el factor del tiempo esperado en un momento que hay $i$ linajes en la genealogía.
...
entonces la esperanza de $T_i$ es
...
$\mathbb{E}(S) = \mathbb{E}[T_{total}] \cdot \mu$

siendo que $\mathbb{E}[T_{total}]$ es la esperanza de la longitud total de rama del árbol coalescente. La longitud se estima sumando cada tiempo esperado de cada $T_i$, multiplicado por la cantidad de linajes que acumulan mutaciones en ese tiempo. Siendo para $n=3, \mathbb{E}[Ttotal]$:

$\mathbb{E}[T_{total}] = \mathbb{E}[t_3] \cdot 3 + \mathbb{E}[t_2] \cdot 2 $

De forma generalizada es
...
La esperanza media simulada es ligeramente mayor que la esperanza teorica, esto se puede explicar por la gran varianza que tiene la distribución geométrica y tambien al tamaño moderado de la muestra (50 simulaciones). 

Finalmente, el histograma confirma que la mayor parte de los resultados se agrupan alrededor de la esperanza teorica (linea roja). Demostrando la validez que tiene el modelo, ya que por la ley de los Grandes Números, a medida que el número de realizaciones en tu muestra (n=50) aumenta, la media de esa muestra $(\bar{X})$ converge al valor de la esperanza teórica $(\mathbb{E}[X])$.
...
El tiempo de coalescencia al ancestro común para $n=2$, se modela con la distribución geométrica. Dado que el tiempo de espera hasta el primer evento de coalescencia en generaciones discretas es una distribucion geométrica. 
...
# Resumen de S
cat("Media simulada S =", mean(S), " y desvio estandar =", sd(S), "\n")
