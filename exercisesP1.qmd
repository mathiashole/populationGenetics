...
Nos gustaría asignar un individuo a una de dos posibles subpoblaciones fuente $S_1$ y $S_2$ según su genotipo
$G_1 = a_1a_1b_1b_2c_1c_1 $, obtenido para tres loci bialélicos no ligados. Se conocen las frecuencias alélicas en
...
¿Cuáles son las probabilidades de observar este genotipo en particular en cada una de las dos
subpoblaciones? 

3. Efecto de la endocría: Calculamos $P(S_1)$ y $P(S_2)$ utilizando las expectativas de HW. ¿Cómo variarían estos
valores y su proporción si ambas subpoblaciones tuvieran cierto grado de endogamia, por ejemplo, $F=0.1$?
Completa la tabla con las fórmulas y los valores adecuados para $P(G_1|S_1)$. Recuerda que $G_1=a_1a_1b_1b_2c_1c_1$

| (header)      | a1a1                     | b1b2                      | c1c1                     | $P(G1 | S1)$                    |
|--------------:|:------------------------:|:-------------------------:|:------------------------:|:-------------------------------:|
|   HW (F = 0)  |       p2                 | 2p(1−p)                   | p2                       |                                 |
|   HW (F = 0)  |                          |                           |                          |                                 |
| Con endogamia |                          |                           |                          |                                 |
...
4. Para este ejercicio, seleccionamos un subconjunto de 20 individuos (10 de Pine Forest y 10 de la
subpoblación Warner Mts.) tomados aleatoriamente de las secuencias generadas en el trabajo citado. De los 701 pares de bases reportados en el artículo, se seleccionaron 620 sitios tras excluir los sitios que no se secuenciaron en todos los individuos. De estos, 50 fueron polimórficos, y solo esos sitios se presentan a continuación. Observamos que el número total de sitios polimórficos (= variables o, usando un término mendeliano, segregantes) en las secuencias es de aproximadamente el 8%. 
...
Con base en los datos mitocondriales, complete la siguiente tabla con las siguientes estadísticas descriptivas
para cada subpoblación:

$A$: número de clases de alelos (= haplotipos distintos)
$S$: número de sitios segregantes
$H$: heterocigosidad
$\pi$: diversidad nucleotídica

Calcular es un poco engorroso, por lo que le recomendamos completar la siguiente tabla como punto de
partida:
...
$H_1$ y $H_2$ → heterocigosidad esperada dentro de cada subpoblación.

$\bar{H_s}$ → heterocigosidad promedio dentro de subpoblaciones.

$\bar{H_t}$ → heterocigosidad total si las subpoblaciones se trataran como una sola (población combinada).

$\dfrac{(p_1 - p_2)^2}{2}$ → exceso de varianza en frecuencias alélicas entre subpoblaciones, que contribuye a la pérdida de heterocigotos al combinar poblaciones.
...
En el segundo caso ($p_1=0.1$, $p_2=0.3$), las subpoblaciones están más diferenciadas: $\bar{H_s} = 0.30$, pero la heterocigosidad total sube a $0.32$. La diferencia es otra vez pequeña, pero se ve más clara la contribución de la varianza de frecuencias ($0.02$).

En el tercer caso ($p_1=0$, $p_2=1$), cada subpoblación es completamente homocigótica ($H_1 = H_2 = 0$), pero al combinar ambas el total tiene la máxima heterocigosidad posible ($\bar{H_t} = 0.50$). Este es el ejemplo extremo del **efecto Wahlund**: dentro de cada subpoblación no hay heterocigotos, pero si juntamos los datos parecería que la población combinada debería tener un 50% de heterocigotos.

¿Cómo se comparan los dos primeros casos?

Caso 1 → más heterocigotos porque $p$ está cerca de 0.5.

Caso 2 → menos heterocigotos porque $p$ está más lejos de 0.5.
...
# Ejercicio 2: Asignando genotipos a subpoblaciones con frecuencias alélicas conocidas - Probabilidad condicional y regla de Bayes
...
# Ejercicio 3: Efecto de la endocría - Probabilidad condicional y regla de Bayes

Fórmulas (Hardy–Weinberg con coeficiente de endogamia F
Para un alelo con frecuencia $p$ y coeficiente de endogamia $F$:
...
En este ejercicio usamos el mismo genotipo del Ejercicio 2:

$G_1=a_1a_1b_1b_2c_1c_1(es decir: AA, Aa, AA)$

y las frecuencias por subpoblación:

$S1$: $p_a=0.3$, $p_b=0.2$, $p_c=0.3$

$S2$: $p_a=0.2$, $p_b=0.25$, $p_c=0.2$

La verosimilitud de observar $G$ en una subpoblación es el producto de probabilidades por locus:

$L=Plocus1\cdot Plocus2\cdot Plocus3$


| (header)      | a1a1                     | b1b2                      | c1c1                     | $P(G1 | S1)$                    |
|--------------:|:------------------------:|:-------------------------:|:------------------------:|:-------------------------------:|
|   HW (F = 0)  |       p2                 | 2p(1−p)                   | p2                       |                                 |
|   HW (F = 0)  |       $0.3^2=0.09$       | $2⋅0.2⋅0.8=0.32$          | $0.3^2=0.09$             | $0.09⋅0.32⋅0.09=0.002592$       |
| Con endogamia |       $p^2+F p(1−p)$     | $2p(1−p)(1−F)$            | $p^2+F p(1−p)$           |                                 |
|  w / F = 0.1  | $0.09+0.1⋅0.3⋅0.7=0.111$ | $2⋅0.2⋅0.8⋅(1−0.1)=0.288$ | $0.09+0.1⋅0.3⋅0.7=0.111$ | $0.111⋅0.288⋅0.111=0.003548448$ |

Sin efecto de endogamia

$P(G_1∣S_1)=0.09⋅0.32⋅0.09 = 0.002592$

Esto indica que el genotipo es poco frecuente bajo HW, ya que combina dos homocigotas y un heterocigota.

Con efecto de endogamia 

$P(G_1∣S_1,F=0.1)=0.111⋅0.288⋅0.111≈0.003548$

Comparado con HW, la probabilidad aumenta porque el genotipo tiene dos homocigotas y la endogamia aumenta la frecuencia de los homocigotas mientras reduce la de los heterocigotas.

Para un genotipo mixto como $G1$ (2 homocigotas + 1 heterocigota), el efecto neto es un ligero aumento de la probabilidad de observar ese genotipo en la población.


# Ejercicio 4: Ejemplo 2 practico 1 HW y F
...
$H = 1 - \sum_{i}P_i^2$
$H = 1 - (0.1)^2 + (0.7)^2 + (0.2)^2$

Warner Mts.

$H = 1 - \sum_{i}P_i^2$
$H = 1 - (0.5)^2 + (0.3)^2 + (0.1)^2 + (0.1)^2$

**Diversidad nucleotídica ($\pi$)**

Pine forest

$\pi = \sum_{i<j}2P_iP_j\pi_{ij}$

haplotipo 1 vs 2 $\pi = 2 (0.1)(0.7)1$

haplotipo 1 vs 3 $\pi = 2 (0.1)(0.2)3$

haplotipo 2 vs 3 $\pi = 2 (0.2)(0.7)2$

Warner Mts.

$\pi = \sum_{i<j}2P_iP_j\pi_{ij}$

haplotipo 1 vs 4 $\pi = 2 (0.5)(0.3)40$

haplotipo 1 vs 5 $\pi = 2 (0.5)(0.1)47$

haplotipo 1 vs 6 $\pi = 2 (0.5)(0.1)48$

haplotipo 4 vs 5 $\pi = 2 (0.3)(0.1)7$

haplotipo 4 vs 6 $\pi = 2 (0.3)(0.1)12$

haplotipo 5 vs 6 $\pi = 2 (0.1)(0.1)5$
